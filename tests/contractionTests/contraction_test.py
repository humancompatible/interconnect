...
def contraction_single(reference_signal, agent_probs, sim_class, it=100, make_plots=False, ax1=None, ax2=None):
    ...
    if make_plots:
        draw_plots(reference_signal, combinations, history, g_history, ax1, ax2)

    r_factor = np.sum(max_g * (1 / len(combinations)))
    return r_factor


def get_factor(reference_signals, agent_probs, sim_class, it=100, make_plots=False):
    """
